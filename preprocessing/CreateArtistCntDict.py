...
stopremove=pd.read_csv("/Users/christopherhedenberg/Downloads/projects/RapThesaurus/ModelOutput/TopicLabels.csv")
typew=stopremove['Unnamed: 3'].tolist()
wordlist = stopremove['word'].tolist()
stopDict={}
for i in range(len(wordlist)):
    stopDict[wordlist[i]]=typew[i]
...
for word in vocab:
    ret =  min([50,len(art_word_dict2[word])-1])
    tmp=[item for item in art_word_dict2[word].items() if stopDict[item[0]]!='stop']
    art_word_dict_sorted2[word] = sorted(tmp,key=lambda tup: tup[1],reverse=True)[:ret]

pickle.dump( art_word_dict_sorted2, open( "/Users/christopherhedenberg/Downloads/projects/RapThesaurus/ModelOutput/ArtWordPickle/ArtWordDictCnt.p", "wb" ) )
